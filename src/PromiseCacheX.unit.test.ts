...
describe('PromiseCacheX', () => {
  ...
  it('should cache promises and return the same pending promise for multiple calls', async () => {
    const fetcher = jest.fn().mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve('delayed-value'), 500)));
  ...
  it('should delete cache entry when promise rejects', async () => {
    ...
    await expect(cache.get('key1', fetcher, { ttl: 5000 })).rejects.toThrow('Fetch failed');
  ...
  describe('LRU Eviction', () => {
    ...
    it('should mark rejected promises as resolved for eviction', async () => {
      ...
      await expect(lruCache.get('key1', () => Promise.reject(new Error('fail')))).rejects.toThrow('fail');
    ...
    it('should handle custom thenables (non-Promise objects with .then)', async () => {
      ...
      const pending = lruCache.get('thenable', () => customThenable as Promise<string>);
